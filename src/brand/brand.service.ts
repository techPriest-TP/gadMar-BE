...
export class BrandService {
  ...
  async findPublic(
    ...
    const brands = await this.prisma.brand.findMany({
      where: {
        ...
        state: options.state,
        deliveryStates: options.deliveryState
          ? { has: options.deliveryState }
          : undefined,
  ...
  async update(id: string, dto: UpdateBrandDto, actor: Actor) {
    ...
    if (actor.role === UserRole.ADMIN && ownerId !== undefined)
      updateData.ownerId = ownerId;
    if (dto.name && dto.name !== brand.name)
      updateData.slug = await this.uniqueSlug(dto.name, id);
    return this.map(
      await this.prisma.brand.update({ where: { id }, data: updateData }),
    );
  }
  ...
  private async findPublicUnique(where: { id?: string; slug?: string }) {
    const brand = await this.prisma.brand.findFirst({
      where: { ...where, verificationStatus: BrandStatus.VERIFIED },
    });
  ...
  private map(brand: any): BrandResponseDto {
    return {
      ...
      warrantyPolicy: brand.warrantyPolicy,
      returnsPolicy: brand.returnsPolicy,
      region: brand.region,
      state: brand.state,
      lga: brand.lga,
      deliveryStates: brand.deliveryStates,
